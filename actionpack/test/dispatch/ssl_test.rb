...
class StrictTransportSecurityTest < SSLTest
  EXPECTED = "max-age=#{18.weeks}"

  def assert_hsts(expected, url: 'https://example.org', hsts: {})
    self.app = build_app ssl_options: { hsts: hsts }
    get url
  ...
  test 'enabled on http:// requests' do
    assert_hsts EXPECTED, url: 'http://example.org'
  end
