*   Add `rake db:version:number` and `rake db:version:file` rake tasks.
    ...
    rake db:version:number prints the current schema version number;
    rake db:version:file prints the current schema's migration file path,
    fall back to print current version number if current schema version
    is ahead of current branch which no migration file can be found.
