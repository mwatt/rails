...
class PostgresqlCollationTest < ActiveRecord::TestCase
  ...
  def setup
    ...
    @connection.create_table :postgresql_collations, force: true do |t|
      ...
      t.text :text_en, collation: 'en_US.UTF-8'
    end
  end
  ...
  test "text column with collation" do
    column = @connection.columns(:postgresql_collations).find { |c| c.name == 'text_en' }
  ...
  test "schema dump includes collation" do
    ...
    assert_match %r{t.text\s+"text_en",\s+collation: "en_US.UTF-8"$}, output
  end
end
