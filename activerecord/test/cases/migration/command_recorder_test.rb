...
module ActiveRecord
  class Migration
    class CommandRecorderTest < ActiveRecord::TestCase
      ...
      def test_invert_remove_foreign_key_with_on_delete
        enable = @recorder.inverse_of :remove_foreign_key, [:dogs, :people, on_delete: :nullify]
        assert_equal [:add_foreign_key, [:dogs, :people, on_delete: :nullify]], enable
      end

      def test_invert_remove_foreign_key_with_on_update
        enable = @recorder.inverse_of :remove_foreign_key, [:dogs, :people, on_update: :cascade]
        assert_equal [:add_foreign_key, [:dogs, :people, on_update: :cascade]], enable
      end
