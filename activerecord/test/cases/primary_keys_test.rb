...
class CompositePrimaryKeyTest < ActiveRecord::TestCase
  ...
  def setup
    @connection = ActiveRecord::Base.connection
    @connection.create_table(:barcodes, primary_key: ["region", "code"], id: false, force: true) do |t|
      t.string :region
      t.integer :code
    end
  end
  ...
  def test_collectly_dump_composite_primary_key
    schema = dump_table_schema "barcodes"
    assert_match %r{create_table "barcodes", primary_key: \["region", "code"\], id: false}, schema
  end
end
