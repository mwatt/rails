...
module ActiveSupport
  ...
  class TimeZone
    ...
    class << self
      ...
      def [](arg)
        case arg
          ...
          when Numeric, ActiveSupport::Duration
            arg = offset_in_minutes arg
            # This returns inconsistent results. See #15209.
            all.find { |z| z.utc_offset == arg }
          else
      ...
      def from_offset(offset, time=Time.now, dst=true)
        offset = offset_in_minutes offset
        all.find { |z| z.utc_total_offset(time, dst) == offset }
      end
    ...
    def utc_total_offset(time=Time.now, dst=true)
