...
In this case, when a user opens the URL `/clients/active`, `params[:status]` will be set to "active". When this route is used, `params[:foo]` will also be set to "bar", as if it were passed in the query string. You will also receive `params[:action]` as "index" and `params[:controller]` as "clients".
