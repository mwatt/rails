...
You may restrict the listing to the routes that map to a particular controller using `-c` option or setting the `CONTROLLER` environment variable:
```bash
$ CONTROLLER=users bin/rake routes
$ bin/rake routes -c users
$ bin/rake routes -c users_controller
$ bin/rake routes -c Users
