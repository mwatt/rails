...
class AppGeneratorTest < Rails::Generators::TestCase
  ...
  def test_application_name_is_detected_if_it_exists_and_app_folder_renamed
    ...
    stub_rails_application(app_moved_root) do
      Rails.application.stub(:is_a?, proc{ Rails::Application }) do
  ...
  def test_spring_binstubs
    ...
    command_check = -> command do
      ...
      when 'exec spring binstub --all'
        @install_called += 1
        assert_equal 1, @install_called, "exec spring binstub --all expected to be called once, but was called #{@install_called} times"
      end
    end
  ...
  def test_after_bundle_callback
    ...
    command_check = -> command do
      ...
      assert sequence.include?(command) && sequence[@called] == command, "commands should be called in sequence #{sequence}"
      @called += 1
    end

    generator([destination_root], template: path).stub(:open, check_open, template) do
      generator.stub(:bundle_command, command_check) do
        generator.stub(:run, command_check) do
          quietly { generator.invoke_all }
        end
      end
    end
